...
import { WASIContext, type WASIContextOptions } from "./wasi-context";
import { type DriveStat, WASIDrive } from "./wasi-drive";
 ...
 *
 */
export class WASI implements Snapshot {
  ...
  getImports(
    ...
    debug?: DebugFn
  ): WebAssembly.ModuleImports & Snapshot {
    const imports = {
    ...
    for (const [name, fn] of Object.entries(imports)) {
      (imports as any)[name] = function (...args: any[]) {
        let ret = (fn as any).apply(this, args);
        if (debug) {
          const argStrings = popDebugStrings();
          ret = debug(name, [...args], ret, argStrings) ?? ret;
        }
        return ret;
      };
    }
